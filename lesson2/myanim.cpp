...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double vx = (double)(rand() % 10);
            double vy = (double)(rand() % 10);
            double vz = (double)(rand() % 10);
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "myanim-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    gmsh::initialize();
    gmsh::model::add("Cali-Dragon_v1_MMU_Body");

    // Считаем STL
    try {
        gmsh::merge("Cali-Dragon_v1_MMU_Body.stl"); 
